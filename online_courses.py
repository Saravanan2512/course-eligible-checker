...
DEPARTMENT_MAPPINGS = {
    ...
    "24": "Arti  ficial Intelligence and Machine Learning (AIML)",
...
# EXCEL_PATH = os.path.join(os.getcwd(), "data", "19m24.xlsx")


EXCEL_19_PATH = os.path.join(os.getcwd(), "data", "19m24.xlsx")
EXCEL_24_PATH = os.path.join(os.getcwd(), "data", "R24m.xlsx")
...
# Determine correct Excel file based on current year
current_year = datetime.now().year % 100
EXCEL_PATH = EXCEL_24_PATH if current_year > 24 else EXCEL_19_PATH


# Utility Functions
def get_student_year(admission_year):
    current_date = datetime.now()
    current_year = current_date.year % 100  # e.g., 2025 → 25
    ...
    # Adjust academic cycle
    academic_year = current_year
    if current_month < 6:
        academic_year -= 1

    student_year = academic_year - admission_year + 1

    if not (1 <= student_year <= 4):
        return "Graduated"

    # Suffix logic
    if 10 <= student_year % 100 <= 20:
        suffix = "th"
    else:
        suffix = {1: "st", 2: "nd", 3: "rd"}.get(student_year % 10, "th")

    return f"{student_year}{suffix} Year"
...
def check_course_eligibility(reg_no, course_name):
    ...
    regulation = "R2019" if int(join_year) <= 23 else "R2024"
